...
fn try_sys_enter_openat(ctx: TracePointContext) -> Result<u32, u32> {
    unsafe {
        ...
        // 读取 dfd (目录文件描述符)
        let dfd = ctx.read_at::<u64>(16);
        match dfd {
            Ok(d) => {
                // AT_FDCWD 通常定义为 -100 (0xFFFFFFFFFFFFFF9C)
                if d == 0xFFFFFFFFFFFFFF9C {
                    info!(&ctx, "dfd: AT_FDCWD (current working directory)");
                } else {
                    info!(&ctx, "dfd: {}", d);
                }
            }
            Err(_) => warn!(&ctx, "read dfd error"),
        }

        // 读取 filename 指针（const char * 是 8 字节的指针）
        let filename_ptr = ctx.read_at::<u64>(24);
        if let Ok(ptr) = filename_ptr {
            if ptr != 0 {
                // 从用户空间读取字符串，最多读取 255 字节（保留 1 字节给 null 终止符）
                let mut filename_buf = [0u8; 256];
